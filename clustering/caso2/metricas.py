...
subset=datos.copy()
usadas = ['Energia Total', 'LLM', 'Experimento', 'Dispositivo']
n_var = len(usadas)
X = subset[usadas]
plot_num = 1
...
#Para cada algoritmo vamos incrementando su bandwidth, epsilon o threshold
...
for name, algorithm in clustering_algorithms:
    ...
    while(k <= 0.7):
        ...
        if(name == 'MeanShift'):
            ...
            cluster_predict = ms.fit_predict(X_normal) #se usa peso para cada objeto (factor de elevación)
        elif(name== 'DBSCAN'):
            dbscan = DBSCAN(eps=k)
            cluster_predict = dbscan.fit_predict(X_normal) #se usa peso para cada objeto (factor de elevación)

        #Comprobamos si hay más de un cluster, en caso de que solo haya 1 no se calculan las medidasb y ni se añaden
...
data.to_excel(f"clustering/caso2/output.xlsx")
...
f.tight_layout()
f.savefig(f"clustering/caso2/metrics.png")
